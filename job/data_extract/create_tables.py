...
#22.07.13 테이블 컬럼 추가 sid, serial, logon_time, module, action, was_id 컬럼명 변경 client_identifier2 -> txn_name 
        commands = (
            """
...
#22.08.01 jdbc_fetch_count 컬럼 추가                
            ...
            """,
            # """
            #     CREATE TABLE AE_WAS_DB_SUMMARY (
            #         time_key timestamp NULL,
            #         txn_name varchar(256) NULL,
            ...
            #         fetched_row_sum int4 NULL,
            #         fetched_row_avg int4 NULL,
            #         fetched_row_max int4 NULL,
...
#22.07.28 ae_txn_sql_summary 테이블 신규 추가            
        ...
        try:
           ...
           logger.error("DB ERROR가 발생했습니다.\n %s", error)        
        
        # commit the changes
        conn.commit()       
        
        # Check Created Table Lists        
        check_query = """
            select n.nspname as schema_name,
                   c.relname as table_name
              from pg_class as c join pg_catalog.pg_namespace as n 
            	                   on n.oid = c.relnamespace 
             where c.relname like 'ae%'
            order by table_name
        """

        cur.execute(check_query)        # Query 수행하기
        fetched_rows = cur.fetchall()   # query로 조회되는 모든 로우 추출        
