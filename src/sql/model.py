...
class ExecuteLogModel(Base):
    ...
    def __repr__(self):
        return (
            "<ExecuteLogModel(seq='{}', execute_name='{}', execute_start_dt='{}', execute_end_dt='{}', "
            "execute_elapsed_time='{}', execute_args='{}', "
            "result='{}', result_code='{}', result_msg='{}', create_id='{}')>".format(
                self.seq,
                self.execute_name,
                self.execute_start_dt,
                self.execute_end_dt,
                self.execute_elapsed_time,
                self.execute_args,
                self.result,
                self.result_code,
                self.result_msg,
...
if __name__ == "__main__":
    ...
    with db.session_scope() as session:
        session.query(ExecuteLogModel).filter(ExecuteLogModel.seq == f"{elm.seq}").update(
            {"result": "Y", "execute_end_dt": "202302281005", "execute_elapsed_time": 80, "result_code": "I000"}
        )
        session.commit()
